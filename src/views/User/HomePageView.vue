  ...
  <div class="home-container">
      ...
      <div class="slider-container" @mouseenter="pauseAutoPlay" @mouseleave="startAutoPlay">
        ...
        <div class="slider-track" :style="{ transform: `translateX(-${currentIndex * 100}%)` }">
          <div v-for="(banner, index) in banners" :key="index" class="slider-item">
            <img :src="banner.image || banner" alt="Banner Promotion" />
          </div>
        </div>
        ...
        <div class="dots-container">
          <span v-for="(banner, index) in banners" :key="index" class="dot" :class="{ active: currentIndex === index }"
    ...
    <section class="product-section container">
      ...
      <div class="section-header">
        <div class="tabs">
          <button v-for="tab in tabsAo" :key="tab" :class="{ active: activeTabAo === tab }" @click="activeTabAo = tab">
            {{ tab }}
          </button>
        </div>
      ...
      <div class="product-grid">
        <div v-for="product in productsAo" :key="product.id" class="product-card" @click="goToProductDetail(product.id)">
    ...
    <section class="product-section container">
      ...
      <div class="product-grid">
        <div v-for="product in productsQuan" :key="product.id" class="product-card" @click="goToProductDetail(product.id)">
          <div class="card-image">
            <img :src="getProductImage(product)" :alt="product.name" />
          </div>
          <div class="card-info">
      ...
      <div class="news-layout" v-if="newsList.length > 0">
        <div class="news-main">
          ...
          <div class="news-content-main">
            ...
            <span class="date">{{ newsList[0].date || newsList[0].create_At }}</span>
          </div>
        </div>
    ...
    <section class="store-section container">
      ...
      <div class="store-layout">
        <div class="map-area">
          <img src="https://upload.wikimedia.org/wikipedia/commons/e/ec/Hanoi_street_map.png" alt="Map Demo" />
          ...
          <div class="store-list-scroll">
            ...
            <div v-if="stores.length === 0">
               <div class="store-item">
                  ...
                  <p><span>📍</span> Số 212A Quang Trung, phường Hà Đông, TP Hà Nội</p>
            ...
            <div v-else class="store-item" v-for="store in stores" :key="store.id">
              <h3>{{ store.name }}</h3>
...
const fetchData = async () => {
  try {
    ...
    if(resCarousel.data.length > 0) banners.value = resCarousel.data.filter(item => item.status === 'active');
    
    if(resProducts.data.length > 0) {
        const all = resProducts.data;
        // Lọc sản phẩm theo tên để chia nhóm
        const ao = all.filter(p => p.name.toLowerCase().includes('áo') || p.name.toLowerCase().includes('đầm'));
        const quan = all.filter(p => p.name.toLowerCase().includes('quần') || p.name.toLowerCase().includes('juyp'));
