  ...
  <div class="admin-container">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <div>
          <h2 class="page-title">Chỉnh Sửa Cửa Hàng</h2>
          <p class="text-muted">Cập nhật thông tin chi nhánh</p>
        </div>
      </div>
        ...
        <div class="card-box">
          ...
          <div class="form-group">
            ...
            <div class="input-wrapper">
                ...
                <input type="text" v-model="form.name" class="form-input pl-40" placeholder="Ví dụ: Orchid Store HCM..." />
            </div>
          </div>
          ...
          <div class="form-group">
            <label class="form-label">
              <input type="checkbox" v-model="form.isMain" :disabled="isMainStoreDisabled">
              Trụ sở chính
            </label>
            <p class="text-xs text-gray-500" v-if="form.isMain">Lưu ý: Chỉ có một trụ sở chính trong hệ thống</p>
          </div>
        </div>
...
// Kiểm tra xem đã có trụ sở chính chưa
const isMainStoreDisabled = computed(() => {
  const stores = loadStoresFromDB();
  const mainStore = stores.find(store => store.isMain && store.id != storeId);
  return !!mainStore && !form.isMain;
});

// Load dữ liệu cửa hàng cần sửa
const loadStoreData = () => {
  const stores = loadStoresFromDB();
  const store = stores.find(s => s.id == storeId); // So sánh string
...
const handleSubmit = () => {
  ...
  // Lấy danh sách cửa hàng hiện tại
  const stores = loadStoresFromDB();
  
  // Tìm index của cửa hàng cần sửa
  const index = stores.findIndex(s => s.id == storeId); // So sánh string
  
  if (index !== -1) {
    // Nếu đánh dấu là trụ sở chính, bỏ đánh dấu của các cửa hàng khác
    if (form.isMain) {
      stores.forEach(store => {
        if (store.id != storeId) { // So sánh string
          store.isMain = false;
        }
      });
    }
    ...
    localStorage.setItem('orchid_stores', JSON.stringify(stores));
    
    alert("Cập nhật cửa hàng thành công!");
    goBack();
  } else {
    alert("Cửa hàng không tồn tại!");
  }
};
